...
<?php while (have_posts()) : the_post();
  $works_intro = get_post_meta(get_the_ID(), 'works_intro', true);
  $works_data  = get_post_meta(get_the_ID(), 'works_data', true);
  $works_quote = get_post_meta(get_the_ID(), 'works_quote', true);
  $works_tags  = get_post_meta(get_the_ID(), 'works_tags', true);
  $works_sections_json = get_post_meta(get_the_ID(), 'works_sections', true);
  $data_items  = $works_data ? json_decode($works_data, true) : [];
  if (!is_array($data_items)) $data_items = [];
  $sections = $works_sections_json ? json_decode($works_sections_json, true) : [];
  ...
  <article class="single-works">
    ...
    <div class="single-works__meta">
      ...
      if ($terms && !is_wp_error($terms)) :
        $cat_color = yuumi_cat_color($terms[0]->slug);
      ?>
        <span class="blog-post-item__cat blog-post-item__cat--<?php echo esc_attr($cat_color); ?>"><?php echo esc_html($terms[0]->name); ?></span>
      <?php endif; ?>
    ...
    <!-- Tags -->
    <?php if ($works_tags) :
      $tag_list = array_map('trim', explode(',', $works_tags));
      $tag_list = array_filter($tag_list);
    ?>
    <div class="single-works__tags">
      <?php foreach ($tag_list as $tag) : ?>
        <span class="single-works__tag">#<?php echo esc_html($tag); ?></span>
      <?php endforeach; ?>
    </div>
    <?php endif; ?>
    ...
    <div class="single-works__body">
      ...
      <!-- Intro -->
      <?php if ($works_intro) : ?>
      <p class="single-works__intro"><?php echo nl2br(esc_html($works_intro)); ?></p>
      <?php endif; ?>

      <!-- Content Sections -->
      <?php if (!empty($sections)) : ?>
      <div class="single-works__sections">
        <?php foreach ($sections as $sec) : ?>
          ...
          <?php if (!empty($sec['image'])) : ?>
            <div class="single-works__section-img">
      ...
      <ul class="single-works__data">
        <?php foreach ($data_items as $item) : ?>
          <?php if (!empty($item['label']) || !empty($item['value'])) : ?>
          <li class="single-works__data-item"><?php echo esc_html($item['label']); ?>：<?php echo esc_html($item['value']); ?></li>
          <?php endif; ?>
        <?php endforeach; ?>
      </ul>
      <?php endif; ?>
      ...
      <!-- Quote -->
      <?php if ($works_quote) : ?>
      <blockquote class="single-works__quote">
        <?php echo nl2br(esc_html($works_quote)); ?>
      </blockquote>
      <?php endif; ?>
    ...
    <div class="share-section">
      <p class="share-section__title">SHARE</p>
    ...
    <nav class="single-works__nav">
      <?php
      $prev_post = get_previous_post();
      $next_post = get_next_post();
      ?>
      <?php if ($prev_post) : ?>
        <a class="single-works__nav-prev" href="<?php echo get_permalink($prev_post); ?>">
          &larr; 前の記事
        </a>
      <?php else : ?>
        <span class="single-works__nav-prev single-works__nav--disabled">&larr; 前の記事</span>
      <?php endif; ?>

      <a class="single-works__nav-back" href="<?php echo esc_url(get_post_type_archive_link('works')); ?>">活動実績一覧へ戻る</a>

      <?php if ($next_post) : ?>
        <a class="single-works__nav-next" href="<?php echo get_permalink($next_post); ?>">
          次の記事 &rarr;
        </a>
      <?php else : ?>
        <span class="single-works__nav-next single-works__nav--disabled">次の記事 &rarr;</span>
      <?php endif; ?>
    </nav>
      ...
      <div class="related-grid">
        ...
        <a href="<?php echo get_permalink($rpost); ?>" class="works-card">
          <div class="works-card__img">
            ...
            <?php else : ?>
              <div style="width:100%;height:100%;background:var(--color-green-pale);"></div>
            <?php endif; ?>
          </div>
          <div class="works-card__meta">
            <?php
            $t = get_the_terms($rpost->ID, 'works_category');
            if ($t && !is_wp_error($t)) :
            ?>
              <span class="works-card__cat"><?php echo esc_html($t[0]->name); ?></span>
            <?php endif; ?>
